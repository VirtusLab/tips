## Relative clauses, its and double pluralized form
### 7 Aug 2020
...
(e.g. has 7 different ways to read _ough_ depending on... nothing in particular) ![](dramat)
...
![](five) unlike in Polish, double consonant (_nn_, _tt+ etc.) is almost never pronounced as double;
...
**infinite** is `'ɪnfɪnət`, which is vowel-wise consistent with **infinity** `ɪn'fɪnəti`, <br/>
but not consistent with **finite** `'faɪnaɪt`, which surprisingly has two `aɪs` ![](infinity)

**major** is `'meɪdʒə(r)`, but **majority** is `mə'dʒɒrəti` (no `eɪ`) ![](hushed) <br/>
**minor** `'maɪnə(r)`, is in turn consistent with **minority** `maɪ'nɒrəti` (`aɪ` in both) ![](relieved)

According to both Martin Odersky and common practice, **Scala** is pronounced as `'skɑ:lə` (no `eɪ`); <br/>
**able** is of course `'eɪb(ə)l` (with `eɪ`), but these vowels get confusingly "swapped" in **scalable**: `'skeɪləb(ə)l` ![](scala)

_ch_ in Greek-derived words like **schema** or **tech(nology)** is pronounced as `k`, not `h` ![](flag-gr) <br/>
...
How about calling it a **makeshift** approach then
...
**Ronseal** is a UK wood paint manufacturer with a slogan
...
**Speak of the devil!** (Polish: _o wilku mowa!_) ![a](evil-parrot)
